...
在 macOS 构建机执行 `bash scripts/build-macos.sh` 后，将生成的真实 DMG/ZIP 放到本目录，再运行 Windows 的 `生成EXE.ps1`。Windows 服务器 EXE 会把本目录作为独立资源嵌入，启动后的“下载苹果服务器/客户端”按钮会自动发布检测到的安装包。
