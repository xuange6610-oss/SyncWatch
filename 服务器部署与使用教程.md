# SyncWatch 服务器部署与使用教程

适用版本：v2.1.5（2026-08-06）

本文面向需要把 SyncWatch 放到 Windows Server、Linux 云服务器、Docker 或内网穿透环境长期运行的用户。文中的目录均以解压后的 `SyncWatch-Server-v2.1.5/` 为例。

## 1. 先理解“程序”和“数据”

独立服务器包只包含运行程序、网页、Android APK 和锁定的生产依赖。所有会持续变化的数据都保存在服务器包根目录的 `SyncWatch-Data/`：
...
因此，“搬迁服务器”必须停止服务后完整移动 `SyncWatch-Data/`，其中也包含 `server-config.json`。不能只复制 `config.json`，不能遗漏隐藏的 `.secrets/`，也不要把两套数据目录直接混合覆盖。

独立服务器不会把数据写回源码目录之外，也不会使用 Windows AppData。桌面 EXE 同样默认把账户、媒体、缓存、日志和密钥写入 EXE 同目录的 `SyncWatch-Data/`。Android 内置服务器受 Android 系统限制，数据位于应用的私有存储中；升级 APK 时应直接覆盖安装，不要先卸载。

## 2. 选择部署方式

| 方式 | 适合场景 | 首次网络要求 | 数据位置 |
| --- | --- | --- | --- |
| Windows Server ZIP | Windows 云主机、家用 Windows 主机 | ZIP 已带 Windows x64 生产依赖，正常启动不需要 npm 下载；仍需预装 Node.js | `服务器目录/SyncWatch-Data` |
| Linux x64 直接部署 | 已有 Node.js 运维环境 | Windows 生成的 ZIP 首次会为 Linux 重装锁定依赖，需要访问 npm | `服务器目录/SyncWatch-Data` |
| Docker Compose（linux/amd64） | Linux x64 云服务器、需要一致环境 | 首次构建需要拉取 Node 镜像和依赖 | 宿主机 `./SyncWatch-Data` |
| 桌面 EXE | Windows 图形界面房主、需要内置 Cloudflare Tunnel | EXE 本身无需 Node.js | `EXE目录/SyncWatch-Data` |
...
SyncWatch 的网页、聊天和同步状态本身很轻，真正消耗资源的是媒体存储、同一影片向多名用户传输，以及不兼容视频的 FFmpeg 转码。服务器不会把一份公网流自动变成运营商组播；同一原画影片有多少名远程观看者，就大致需要多少份服务器出网带宽。
...
推荐成品架构：`域名 -> HTTPS 的 Caddy/Nginx -> 127.0.0.1:5000 的 SyncWatch -> 独立 SyncWatch-Data 数据盘`。
...
- Windows：`D:\SyncWatch-Server\`
- Linux：`/opt/syncwatch/`

不要直接在 ZIP 压缩包内运行，也不建议放进 Windows 的 `Program Files` 后以普通用户运行，因为程序必须能创建和修改 `SyncWatch-Data/`。目录所在磁盘要预留影片、缩略图、回收数据和备份所需空间。

解压后至少应看到：

```text
SyncWatch-Server-v2.1.5/
├─ server/
├─ public/
...
默认端口为 `5000`。当指定端口被占用时，服务器会自动随机选择一个可用端口，并把实际端口写入 `SyncWatch-Data/服务器运行信息.txt`。

端口优先级从高到低为：

1. 命令行 `--port`
2. 环境变量 `PORT`
3. `SyncWatch-Data/server-config.json` 的 `port`

最简单的方法是修改 `SyncWatch-Data/server-config.json`。首次启动时程序会自动生成该文件：
...
2. 解压服务器 ZIP 到可写目录。
3. 先启动一次生成数据目录，再修改 `SyncWatch-Data/server-config.json` 并重启服务。
4. 双击 `start-server.cmd`，或在 PowerShell 中执行：

   ```powershell
   Set-Location 'D:\SyncWatch-Server'
   .\start-server.ps1
   ```

5. 控制台显示地址后，先在服务器本机打开 `http://127.0.0.1:端口`。
6. 私密房主入口同时写入 `SyncWatch-Data/服务器运行信息.txt`。设置了 `publicUrl` 时，该入口会使用公网地址；未设置时默认是 `127.0.0.1`。远程管理者可在已经正确配置 `publicUrl`/`allowedHosts` 和反代后，把链接的地址部分换成实际公网地址，但必须原样保留 `#host=` 后的令牌。包含 `#host=` 的完整链接只能由服务器管理者使用，不能发给普通用户。
...
```powershell
New-NetFirewallRule -DisplayName 'SyncWatch TCP 5000' -Direction Inbound -Action Allow -Protocol TCP -LocalPort 5000
```
...
- 程序：`powershell.exe`
- 参数：`-NoProfile -ExecutionPolicy Bypass -File "D:\SyncWatch-Server\start-server.ps1"`
- 起始于：`D:\SyncWatch-Server`
...
如果最后两条命令成功，说明 Docker 和 Compose 已就绪。把 `SyncWatch-Server-v2.1.5.zip` 上传到服务器后解压：

```bash
sudo apt install -y unzip
sudo mkdir -p /opt/syncwatch
sudo unzip ~/SyncWatch-Server-v2.1.5.zip -d /opt/syncwatch
cd /opt/syncwatch
```
...
Compose 会把宿主机的 `./SyncWatch-Data` 绑定到容器的 `/app/SyncWatch-Data`，并给安全关闭保留 90 秒。删除或重建容器不会删除宿主机数据；但删除服务器目录、误删绑定目录或使用错误的工作目录仍会造成数据丢失。
...
4. 把 `SyncWatch-Data/server-config.json` 的 `publicUrl` 设置为最终 HTTPS 地址，并把域名加入 `allowedHosts`。
5. 重启 SyncWatch 和反向代理。

网页通过 HTTPS 打开时，Socket.IO/WebSocket 也必须经过同一个 HTTPS 域名。不要让 HTTPS 页面连接明文 `ws://` 或另一个未受信任端口，否则浏览器会阻止连接。

如果域名使用 Cloudflare DNS：

- 灰云“仅 DNS”不会经过 Cloudflare HTTP 代理，最适合需要上传几百 MB 或数 GB 视频的 SyncWatch。
- 橙云“已代理”支持 WebSocket，但请求体仍受 Cloudflare 套餐上传上限约束。官方当前列出的默认上限为 Free/Pro 100 MB、Business 200 MB、Enterprise 500 MB；超过时会在到达 SyncWatch 前返回 413。
- 因此需要大文件上传时，应把 SyncWatch 记录设为“仅 DNS”，或使用支持更大上传上限的企业配置。Cloudflare Tunnel 同样经过 Cloudflare 边缘，不能把应用的 32 GiB 上限误当成代理也能接受 32 GiB。
...
以下最终配置假定 SyncWatch 在 `127.0.0.1:5000`，并且证书已经位于示例中的 Let’s Encrypt 路径。证书尚未签发时可先使用 Caddy，或按系统发行版安装 Certbot 并为域名签发证书，再启用 443 配置。先在 Nginx 的 `http {}` 级别加入连接升级映射：
...
server {
    ...
    # 让 SyncWatch 自己执行 32 GiB 媒体上限和管理员限制，避免 multipart
...
`client_max_body_size 0` 表示不让 Nginx 另加请求体大小限制；SyncWatch 管理界面的“最大文件 MB”设置为 0 表示不增加用户配置限制，但服务端仍有 32 GiB 安全上限，语音消息上限为 25 MB。上传时长设置为 0 表示不增加管理员自定义限制，但单次 HTTP 请求仍有 2 小时安全上限。
...
确保 80/443 可从公网访问，DNS 已指向本机，并设置 SyncWatch 的 `publicUrl` 与 `allowedHosts`。Caddy 标准 `reverse_proxy` 不需要手写 `Upgrade`/`Connection` 头。首次申请证书失败时，先检查 DNS 是否已经指向当前公网 IP、80/443 是否被安全组和防火墙放行，以及同一域名是否被另一台服务器占用。
...
- HTTP、Socket.IO polling 和 WebSocket 都转发到同一个 SyncWatch 端口。
...
1. 使用 `SyncWatch-Data/服务器运行信息.txt` 中带 `#host=` 的私密入口打开服务器。若文件里是 `127.0.0.1` 而管理浏览器不在服务器本机，请先正确设置公网地址和允许主机，再按第 5 章的方法保留令牌并替换地址部分。
...
普通用户不要使用房主链接。房主令牌文件属于服务器密钥；如果泄露，应先停止服务，备份后删除 `SyncWatch-Data/.secrets/server-host-token.txt`，再启动服务器生成新令牌，并检查账户与操作历史。旧的 `#host=` 链接随后失效。
...
SyncWatch 使用真实 `smtp.qq.com:465`、TLS 1.2 或更高版本发送邮件。这里需要的是 QQ 邮箱的 SMTP 授权码，不是 QQ 登录密码。
...
支持 `qq.com`、`foxmail.com` 和 `vip.qq.com` 发件地址。授权码使用 AES-256-GCM 加密后写入 `config.json`，解密密钥单独保存在 `SyncWatch-Data/.secrets/mail.key`；浏览器不会读取已保存的授权码。更换发件邮箱时必须填写新授权码。
...
Copy-Item -LiteralPath 'D:\SyncWatch-Server\SyncWatch-Data' -Destination 'E:\Backup\SyncWatch-Data-2026-08-05' -Recurse
```

```bash
sudo mkdir -p /srv/backup
sudo cp -a /opt/syncwatch/SyncWatch-Data /srv/backup/SyncWatch-Data-2026-08-05
```

`server-config.json` 已位于 `SyncWatch-Data/` 中，完整备份数据目录时会一起保存。备份后至少检查文件数量、总大小，并抽查 `config.json`、`chat-history.jsonl`、`.secrets/` 和大型影片；重要服务器可再生成 SHA-256 清单。
...
1. 停止旧服务器。
2. 完整复制整个服务器目录，或把新服务器程序和旧 `SyncWatch-Data/` 一起复制到目标位置。
...
1. 停止服务器。
2. 先把当前 `SyncWatch-Data/` 改名或完整备份，避免误操作无法回退。
3. 用同一时间点的完整备份替换整个 `SyncWatch-Data/`。
4. 不要把旧、新两套目录逐个文件混合；这可能造成媒体索引、聊天、回收记录和密钥不一致。
5. 启动后进行登录、房间、播放、聊天、QQ 测试邮件和历史回溯检查。

旧版桌面程序可能在 `%APPDATA%\sync-watch-lan\data` 留有数据。新版 EXE 首次启动会在目标 `SyncWatch-Data/` 不存在有效配置时复制并校验旧数据，但会保留旧目录作为安全备份。确认新目录完整运行前绝不能删除旧 AppData 数据。
...
1. 退出旧 EXE，确认进程已经结束。
2. 备份整个 `SyncWatch-Data/`，其中已经包含 `server-config.json`。
...
1. 停止旧服务并备份旧 `SyncWatch-Data/`。
2. 解压新版 ZIP 到新目录。
3. 新版 ZIP 自带一个只含说明文件的 `SyncWatch-Data/` 占位目录。先确认它没有真实数据，再把它改名为 `SyncWatch-Data.placeholder/`；不要直接把旧目录移动到这个同名目录中，否则可能形成错误的 `SyncWatch-Data/SyncWatch-Data/` 嵌套。
4. 把旧版完整的 `SyncWatch-Data/` 作为一个目录移动到新版根目录，确认最终路径正好是 `新版目录/SyncWatch-Data/config.json`。旧版根目录若还有 `server-config.json`，首次启动会自动迁移到数据目录；禁止把新旧两套数据逐文件合并覆盖。
5. 启动新版并验收；确认无误后再归档旧程序目录，并删除已经确认无用的 `SyncWatch-Data.placeholder/`。

Docker 升级时，先备份宿主机 `SyncWatch-Data/`，再执行 `docker compose up -d --build`。绑定目录不会因容器重建而改变。

## 20. 日志和运行状态

- 前台启动：主要日志输出到控制台。
- 独立服务器：地址、端口和房主入口写入 `SyncWatch-Data/服务器运行信息.txt`。
- systemd：使用 `journalctl -u syncwatch -f`。
- Docker：使用 `docker compose logs -f`。
- 桌面 EXE：缓存、日志和崩溃目录位于 `SyncWatch-Data/cache`、`logs`、`crash-dumps`。
...
指定端口已被其他程序占用。停止占用进程或修改端口；SyncWatch 不会自动换端口。

### 外网能打开页面，但登录、同步或聊天断开

通常是 WebSocket 反代、代理超时或负载均衡配置问题。先使用本文 Nginx/Caddy 配置，确保同一客户端的 HTTP 与 Socket.IO 到达同一个 SyncWatch 实例。当前程序不是多节点共享状态架构，不要同时启动多个实例指向同一数据目录。

### 上传返回 413

检查 Nginx `client_max_body_size`、SyncWatch 管理设置和 32 GB 服务端安全上限。反代还应关闭请求缓冲并延长上传超时。
...
程序启动到了新的空 `SyncWatch-Data/`。停止服务，把原目录完整放到新程序根目录，尤其不能遗漏隐藏的 `.secrets/`。不要让两个服务器同时使用同一网络共享数据目录。
...
先检查是否同时运行了桌面 EXE、独立服务器、Docker 容器或另一个端口的 Node 进程，并正常关闭原实例。异常崩溃且原 PID 已不存在时程序会自动回收旧锁；只有在确认所有 SyncWatch 进程都已停止后，才可备份并人工处理损坏的 `.syncwatch-instance.lock/`。运行中的锁绝不能强删。

### 影片卡片存在，但播放或下载返回 `MEDIA_FILE_UNAVAILABLE`

服务器会在磁盘、挂载或权限暂时异常时保留媒体索引和队列，而不是删除元数据。检查 `SyncWatch-Data/uploads/` 是否已完整挂载、服务账户是否有读取权限、文件是否复制完成；原文件恢复到相同文件名后即可重新播放，无需重建影片库。
...
- 已制定 `SyncWatch-Data/` 定期离线备份和恢复演练计划。
- 单个数据目录只由一个 SyncWatch 实例写入；程序会用 `.syncwatch-instance.lock/` 阻止同一目录被不同端口或不同启动方式同时打开。
...
9. 重启服务后数据仍存在，`SyncWatch-Data/服务器运行信息.txt` 更新。
10. 备份可以在隔离目录恢复并独立启动。
