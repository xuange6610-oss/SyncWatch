# SyncWatch 云端视频与商业部署说明

## 已实现的运行方式

当前版本已经支持在影片库点击“添加云端视频”，保存腾讯云 COS / 阿里云 OSS 的 HTTPS 视频直链。服务器只保存视频名称、直链、封面/时长等元数据；客户端播放时直接访问对象存储，播放、暂停、拖动和房间状态仍由现有 Socket.IO 服务同步，因此视频流量不会经过 SyncWatch 服务器。
...
- 私有 Bucket 使用 CDN 签名 Cookie 或足够长的签名 URL。不要把永久 SecretId、SecretKey、AccessKey 写入前端或 SyncWatch 数据文件。
...
当前桌面/局域网版本继续使用 `SyncWatch-Data`，这样离线部署、Windows EXE 和 Android 本机服务器不依赖数据库。面向公网多实例部署时，建议按下面方式演进，而不是让单机 JSON 被多个容器同时写入：
