...
适用版本：v2.1.7（2026-08-06）

本文面向需要把 SyncWatch同步观影 放到 Windows Server、Linux 云服务器、Docker 或内网穿透环境长期运行的用户。文中的目录均以解压后的 `SyncWatch同步观影-Server-v2.1.7/` 为例。
...
推荐成品架构：`域名 -> HTTPS 的 Caddy/Nginx -> 127.0.0.1:5000 的 SyncWatch同步观影 -> 独立 SyncWatch同步观影-Data 数据盘`。
...
5. 创建 `.env`，把容器端口绑定到 `127.0.0.1`，避免 5000 直接暴露公网。
6. 执行 `docker compose up -d --build`，确认服务器本机能访问 `http://127.0.0.1:5000/api/public-config`。
...
```text
SyncWatch同步观影-Server-v2.1.7/
├─ server/
├─ public/
├─ mobile/SyncWatch同步观影-v2.1.7.apk
├─ node_modules/
├─ SyncWatch同步观影-Data/
│  └─ server-config.json
...
```powershell
New-NetFirewallRule -DisplayName 'SyncWatch同步观影 TCP 5000' -Direction Inbound -Action Allow -Protocol TCP -LocalPort 5000
```

如果以后改端口，应修改或删除旧规则。使用同机 Nginx/Caddy 反代时，公网只需开放 80/443，5000 可限制为本机或内网访问。
...
如果最后两条命令成功，说明 Docker 和 Compose 已就绪。把 `SyncWatch同步观影-Server-v2.1.7.zip` 上传到服务器后解压：

```bash
sudo apt install -y unzip
sudo mkdir -p /opt/syncwatch
sudo unzip ~/SyncWatch同步观影-Server-v2.1.7.zip -d /opt/syncwatch
cd /opt/syncwatch
```
...
`SYNCWATCH_BIND_ADDRESS=127.0.0.1` 适用于同机 Caddy/Nginx 反代，公网只能通过 80/443 访问；如果确实需要让局域网设备直接打开 `http://服务器IP:5000`，改为 `0.0.0.0`，并同步配置安全组和防火墙。Docker 发布端口可能绕过部分 UFW 直觉规则，因此生产反代场景优先绑定到 `127.0.0.1`。
...
- 直接以 `http://IP:5000` 访问：放行 TCP 5000。
- 使用 Nginx/Caddy HTTPS：放行 TCP 80 和 443，不建议向公网放行 5000。
...
- 使用橙云时，到 Cloudflare 控制台确认 WebSockets 已启用，并只代理 Cloudflare 支持的 HTTP/HTTPS 端口。最简单的是只对外使用 443，内部仍反代到 5000。

## 10. Nginx 反向代理与 WebSocket

以下最终配置假定 SyncWatch同步观影 在 `127.0.0.1:5000`，并且证书已经位于示例中的 Let’s Encrypt 路径。证书尚未签发时可先使用 Caddy，或按系统发行版安装 Certbot 并为域名签发证书，再启用 443 配置。先在 Nginx 的 `http {}` 级别加入连接升级映射：
...
2. 在服务器设备登录页可直接选择“服务器超级管理员登录”，只输入初始密码 `admin888`。
3. 首次登录先阅读并同意使用协议，然后可直接填写新密码和确认密码；不需要再次输入 `admin888`。也可点“暂不更改”先进入，但建议在开放公网前完成更换。
4. 超级管理员不填写房间号时会进入一个临时管理房间，退出后自动删除；填写已有房间号时进入对应正式房间，不会重复生成删不完的房间记录。
5. 普通账号注册不要求房间号或房间密码，但注册完成后不能空房号直接进入。应点击“新建房间”创建自己的第一个房间，或填写/选择一个在线房间号加入。
