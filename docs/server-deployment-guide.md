...
适用版本：v2.1.6（2026-08-06）

本文面向需要把 SyncWatch同步观影 放到 Windows Server、Linux 云服务器、Docker 或内网穿透环境长期运行的用户。文中的目录均以解压后的 `SyncWatch同步观影-Server-v2.1.6/` 为例。
...
```text
SyncWatch同步观影-Server-v2.1.6/
├─ server/
├─ public/
├─ mobile/SyncWatch同步观影-v2.1.6.apk
├─ node_modules/
├─ SyncWatch同步观影-Data/
│  └─ server-config.json
...
如果最后两条命令成功，说明 Docker 和 Compose 已就绪。把 `SyncWatch同步观影-Server-v2.1.6.zip` 上传到服务器后解压：

```bash
sudo apt install -y unzip
sudo mkdir -p /opt/syncwatch
sudo unzip ~/SyncWatch同步观影-Server-v2.1.6.zip -d /opt/syncwatch
cd /opt/syncwatch
```
