...
Release 同时提供 MSI 安装包和独立命令行 EXE。普通新手优先下载 [`cloudflared-windows-x64-installer.msi`](https://github.com/xuange6610/SyncWatch/releases/download/2.1.7/cloudflared-windows-x64-installer.msi)；32 位 Windows 下载 [`cloudflared-windows-x86-installer.msi`](https://github.com/xuange6610/SyncWatch/releases/download/2.1.7/cloudflared-windows-x86-installer.msi)。`cloudflared-windows-x64.exe` 是命令行工具，不是安装程序，直接双击只会打开黑色终端且不会出现安装向导。正式 SyncWatch 服务器 EXE 已内置同一组件，无需重复安装。
