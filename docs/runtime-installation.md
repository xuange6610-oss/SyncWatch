...
这两个工具用途不同。普通用户安装 `SyncWatch-v2.1.8-Full-Offline-Installer-x64.exe` 或运行 `SyncWatch-Standard-Server-Portable` 时，不需要另外安装 Node.js，也不需要另外下载 cloudflared；服务器包已经包含所需运行时。只有手工诊断、公网部署、源码开发或独立服务器才需要本教程。
...
Release 同时提供 MSI 安装包和独立命令行 EXE。普通新手优先下载 [`cloudflared-windows-x64-installer.msi`](https://github.com/xuange6610/SyncWatch/releases/download/2.1.8/cloudflared-windows-x64-installer.msi)；32 位 Windows 下载 [`cloudflared-windows-x86-installer.msi`](https://github.com/xuange6610/SyncWatch/releases/download/2.1.8/cloudflared-windows-x86-installer.msi)。`cloudflared-windows-x64.exe` 是命令行工具，不是安装程序，直接双击只会打开黑色终端且不会出现安装向导。正式 SyncWatch 服务器 EXE 已内置同一组件，无需重复安装。
