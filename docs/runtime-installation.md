...
Release 中的 `cloudflared-windows-x64.exe` 是独立诊断工具，正式服务器 EXE 内已经内置同一组件。

1. 从 SyncWatch Release 或 [Cloudflare 官方 Release](https://github.com/cloudflare/cloudflared/releases/latest) 下载 Windows x64 文件。
2. 放入单独文件夹，并重命名为 `cloudflared.exe`。
3. 在该文件夹空白处按住 Shift 并点击鼠标右键，选择“在终端中打开”。
4. 运行版本检查：
...
5. 先启动 SyncWatch 服务器，确认浏览器可以打开 `http://127.0.0.1:5000`。
6. 手工创建临时地址：
...
7. 等待终端显示 `https://随机名称.trycloudflare.com`，用另一网络的手机测试登录和媒体播放。
8. 按 `Ctrl+C` 停止 Tunnel。终端关闭后临时地址会失效，下次地址可能不同。
