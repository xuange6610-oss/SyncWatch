...
双击正式发布目录 `release/windows-server/SyncWatch同步观影-v2.1.8.exe` 中的服务器程序。首次启动时，Windows 防火墙如有提示，请允许“专用网络”。程序会在 EXE 同目录创建 `SyncWatch同步观影-Data/`，账号、设置、媒体、缩略图、字幕、语音、聊天、缓存和密钥全部保存在这里；把整个程序文件夹移动到另一台电脑或云服务器时，数据会随文件夹一起移动。
...
3. 普通用户可直接注册，注册时不要求房间号或房间密码；登录时房间号留空会进入退出即删除的临时房间，也可创建正式房间或填写/选择在线房间。
...
1. 服务器设备登录页可直接选择“服务器超级管理员登录”，只输入管理员密码。首次使用 `admin888` 后直接设置新密码和确认密码。
2. 不填写房间号时进入临时管理房间，退出后自动删除；填写房间号时进入指定正式房间。
...
7. 未登录时也可打开顶栏“服务器设置”，直接输入超级管理员账号和密码验证；成功后验证框自动消失。超级管理员也可修改当前房间号。
...
- 上传任务可折叠到后台继续处理。“我的影片”和“媒体处理进度”会实时显示转换中、等待及已完成数量；兼容转换默认同时运行 3 个任务，服务器管理员可在 1-8 之间即时调整，并可直接打开转换文件目录。
...
- 顶栏“生成 APK”会下载本 EXE 内置并经过签名校验的 `SyncWatch同步观影-v2.1.8.apk`。安装后输入电脑端显示的 HTTP 局域网地址或 HTTPS 公网地址，即可使用与网页端一致的房间、播放、聊天、上传、全屏和账户功能。
