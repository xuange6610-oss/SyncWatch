...
该脚本会先构建并验证 APK，再执行接口、硬化、媒体、Electron、同步、主进程、穿透和成品测试，最后原子替换 `release/windows-server/SyncWatch同步观影-v2.1.6.exe`，并同步生成 Windows 客户端和服务器部署包。
...
产物为 `release/server-deployment/SyncWatch同步观影-Server-v2.1.6.zip`，包含服务器源码、网页、APK、启动脚本、Docker 文件、部署文档和锁定的 Windows x64 生产依赖，不包含 Electron、测试目录、开发缓存和 Android 私钥。
