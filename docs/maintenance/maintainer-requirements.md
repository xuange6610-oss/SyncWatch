...
- 每个正式版本严格遵守 [Release 资产清单](../release/release-manifest.md)：26 个维护者真实资产，加 GitHub 自动生成的 2 个源码归档，页面共 28 个可见文件。
...
- 永久说明必须保留上一正式版本已有的完整结构，包括“普通用户怎么选”“跨平台完整套装”“一键运行包含什么”“macOS”“架构支持边界”“cloudflared 独立工具”“Node.js 官方环境包”“首次启动”“安全与许可”。
...
- 本次版本发布必须一次完成：先完成所有代码、配置、文档和测试改动，再在本地运行回归测试与必要 build；确认真实成品、SHA-256、平台架构和 26+2 文件清单均通过后，才删除当前版本 Release 中的旧维护者资产并完整上传新资产。中途不能先上传部分文件，也不能用占位文件凑数；任何一项未验证都必须暂停发布并标记未完成。
...
- 当前正式版本：`v2.1.9`；`main`、`v2.1.9` tag、Release 标题和发布说明使用同一版本标识。
- 当前仓库：`https://github.com/xuange6610/SyncWatch`。
- 当前 Pages：`https://xuange6610.github.io/SyncWatch/`。
- v2.1.7、v2.1.8 与 v2.1.9 均已验证为 26 个维护者资产，加 2 个 GitHub 源码归档，每个版本共 28 个可见文件；后续正式版本继续沿用同一门禁。
