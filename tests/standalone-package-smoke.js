...
if (!/path\.join\(ROOT_DIR,\s*['"]mobile['"],\s*['"]SyncWatch同步观影-v2\.1\.7\.apk['"]\)/.test(fs.readFileSync(path.join(root, 'server-standalone.js'), 'utf8'))) {
  throw new Error('standalone server must expose the packaged SyncWatch同步观影-v2.1.7.apk');
}
...
const packagedClientPath = path.join(root, 'SyncWatch同步观影-Client-v2.1.7.exe');
...
if (!fs.readFileSync(path.join(root, 'Dockerfile'), 'utf8').includes('COPY SyncWatch同步观影-Client-v2.1.7.exe ./client/SyncWatch同步观影-Client-v2.1.7.exe')) {
