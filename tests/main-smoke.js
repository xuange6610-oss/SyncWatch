...
const driveDRoot = electronSettings.resolveApplicationRoot({ portableExecutableFile: 'D:\\SyncWatch同步观影\\SyncWatch同步观影-v2.1.6.exe' });
const driveERoot = electronSettings.resolveApplicationRoot({ portableExecutableFile: 'E:\\SyncWatch同步观影\\SyncWatch同步观影-v2.1.6.exe' });
...
const packagedServerPath = path.join(dataDir, 'SyncWatch同步观影-v2.1.6.exe');
const packagedClientPath = path.join(dataDir, 'SyncWatch同步观影-Client-v2.1.6.exe');
...
assert.equal(electronSettings.resolveClientDownloadPath({ isPackaged: true, portableExecutableFile: path.join(dataDir, 'missing', 'SyncWatch同步观影-v2.1.6.exe') }), '');
const macArtifacts = path.join(dataDir, 'mac');
fs.mkdirSync(macArtifacts);
const macServerArm64 = path.join(macArtifacts, 'SyncWatch同步观影-服务器-v2.1.6-arm64.dmg');
const macServerX64Zip = path.join(macArtifacts, 'SyncWatch同步观影-服务器-v2.1.6-x64.zip');
const macClientX64 = path.join(macArtifacts, 'SyncWatch同步观影-客户端-v2.1.6-x64.dmg');
const macClientX64Zip = path.join(macArtifacts, 'SyncWatch同步观影-客户端-v2.1.6-x64.zip');
