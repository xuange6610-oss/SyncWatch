...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'syncwatch-electron-v2.1.5-'));
...
async function run() {
  ...
  // v2.1.5 may explicitly explain an unknown account instead of promising a
...
app.whenReady().then(async () => {
  ...
  try {
    await run();
    console.log('\nElectron v2.1.5 渲染验收全部通过。');
  } catch (error) {
    exitCode = 1;
    console.error('\nElectron v2.1.5 渲染验收失败:', error);
  }
  try { await finishTest(exitCode); }
  catch (error) { console.error('\nElectron v2.1.5 清理失败:', error); app.exit(1); }
}).catch((error) => { console.error('\nElectron v2.1.5 启动失败:', error); app.exit(1); });
