...
async function main() {
  ...
  try {
    ...
    assert.equal(publicConfig.version, 'v2.1.8');
    ...
    assert.equal(serverInfo.version, 'v2.1.8');
    console.log('SyncWatch同步观影 v2.1.8 backend admin/mail/media regression passed');
  } finally {
...
main().catch((error) => {
  console.error('SyncWatch同步观影 v2.1.8 backend regression failed:', error);
  process.exitCode = 1;
});
