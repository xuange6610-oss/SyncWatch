...
async function main() {
  ...
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'syncwatch-v2.1.5-integration-'));
  const publicDir = path.resolve(__dirname, '..', 'public');
  const clients = [];
  let server;
  let checks = 0;
  ...
  try {
    ...
    assert.equal(config.response.status, 200); assert.equal(config.payload.version, 'v2.1.5');
    ...
    check('公开配置为 v2.1.5，默认账户上传额度和时长配置已返回');
    ...
    // v2.1.5 explicitly reports an unknown account/mailbox. Keep accepting the
