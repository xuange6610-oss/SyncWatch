...
async function main() {
  ...
  const files = {
    serverX64: path.join(artifactsDir, 'SyncWatch同步观影-服务器-v2.1.7-x64.zip'),
    serverArm64: path.join(artifactsDir, 'SyncWatch同步观影-服务器-v2.1.7-arm64.dmg'),
    clientX64: path.join(artifactsDir, 'SyncWatch同步观影-客户端-v2.1.7-x64.dmg'),
    clientArm64: path.join(artifactsDir, 'SyncWatch同步观影-客户端-v2.1.7-arm64.zip')
  };
  ...
  const discovered = macDistribution.createMacDistribution({
    kind: 'server', version: 'v2.1.7', roots: [artifactsDir], includeDefaultRoots: false,
  ...
  const rejectedLocalArtifacts = macDistribution.createMacDistribution({
    kind: 'server', version: 'v2.1.7', roots: [], includeDefaultRoots: false, env: {},
    configured: {
  ...
  const bridged = macDistribution.createMacDistribution({
    kind: 'client', version: 'v2.1.7', legacyPaths: normalizedBridge,
  ...
  try {
    server = await startSyncWatchServer({
      ...
      macClientDownloadUrls: { arm64: { dmg: 'https://downloads.example.test/SyncWatch同步观影-客户端-v2.1.7-arm64.dmg' } }
    });
