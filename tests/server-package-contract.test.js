...
assert.match(buildScript, /Join-Path\s+\$PSScriptRoot\s+['"]release\\windows-client\\SyncWatch同步观影-Client-v2\.1\.7\.exe['"]/);
assert.doesNotMatch(buildScript, /Join-Path\s+\$PSScriptRoot\s+['"]SyncWatch同步观影-Client-v2\.1\.7\.exe['"]/,
...
try {
  ...
  fs.writeFileSync(path.join(source, 'mac', 'SyncWatch同步观影-unrelated-v2.1.7-x64.zip'), 'unrelated');
