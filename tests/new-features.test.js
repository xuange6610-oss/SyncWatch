...
async function main() {
  ...
  try {
    ...
    const topbarOrder = [
      'aiWorkbenchBtn', 'masterMuteBtn', 'quickDissolveRoomBtn', 'newRoomBtn', 'switchRoomBtn', 'lanScanBtn',
    ...
    assert.match(clientLauncherSource, /首次连接时会在本机创建 SyncWatch-Data/);
    assert.match(electronServerSource, /app\.setName\(['"]同步观影['"]\)/);
    assert.match(electronServerSource, /setAppUserModelId\(['"]com\.tangjingxuan\.syncwatch\.server['"]\)/);
    assert.match(electronClientSource, /app\.setName\(['"]同步观影['"]\)/);
    ...
    assert.ok(!packageManifest.build.extraResources.some((entry) => entry.from === 'SyncWatch-Client-v2.1.5.exe'));
    assert.equal(packageManifest.build.portable.artifactName, 'SyncWatch同步观影-v2.1.5.exe');
    assert.equal(packageManifest.build.win.executableName, '同步观影');
