...
assert.ok(!packageManifest.build.extraResources.some((entry) => ['mac', 'mobile', 'SyncWatch同步观影-Client-v2.1.6.exe'].includes(String(entry.from || ''))),
...
assert.match(standalone, /SyncWatch同步观影-服务器-v2\.1\.6/);
assert.match(standalone, /SyncWatch同步观影-客户端-v2\.1\.6/);
assert.match(dockerfile, /COPY SyncWatch同步观影-Client-v2\.1\.6\.exe \.\/client\/SyncWatch同步观影-Client-v2\.1\.6\.exe/,
  'Docker deployment must include the Windows client download artifact');
assert.ok(dockerignore.indexOf('!SyncWatch同步观影-Client-v2.1.6.exe') > dockerignore.indexOf('*.exe'),
