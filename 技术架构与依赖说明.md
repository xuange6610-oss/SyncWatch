...
SyncWatch 是一套可离线部署的多人同步观影系统。Windows 桌面端同时承担图形客户端和服务器主机；独立服务器包可部署到 Windows Server、Linux x64 或 Docker；Android APK 既能作为客户端，也内置同一套 Node.js 服务端供手机热点或局域网使用。
...
| `server-standalone.js` | 独立服务器启动入口；读取端口和公网配置；生成并保存服务器主机令牌；定位 `SyncWatch-Data`；写入服务器运行信息；处理 SIGINT/SIGTERM 安全关闭。 |
...
桌面端和独立服务器统一使用程序旁的 `SyncWatch-Data/`。主要内容如下：
...
产物为 `release/server-deployment/SyncWatch-Server-v2.1.5.zip`，包含服务器源码、网页、APK、启动脚本、Docker 文件、部署文档和锁定的 Windows x64 生产依赖，不包含 Electron、测试目录、开发缓存和 Android 私钥。
...
`SyncWatch-Data/` 是运行数据，不属于空白源码发布包；重新发布前应删除真实账户、媒体、聊天、密钥、缓存和旧构建产物。
